    ...
    def get_servers_input(self):
        ...
        while True:
            ...
            if '***identified***' in sockdata:
                ...
                a=self.S.parse_server_str(sockdata)
                if self.debug:
...
class ServerState():
    ...
    def parse_server_str(self, server_string):
        ...
        for i in sslisted:
            w= i.split(' ')
            lis.append(destringify(w[1:]))
            self.d[w[0]]= destringify(w[1:])
        a=list(chain.from_iterable(item if isinstance(item,Iterable) and
                    not isinstance(item, str) else [item] for item in lis))
        return a
...
class DriverAction():
    ...
    def fancyout(self):
        ...
        for k in sorted(od):
            ...
            out+= "%s: %s\n" % (k,strout)
        return out
def stuck_check(sx,a,p,fwdtsen):
    if fwdtsen > 5: 
        return False
    if abs(a)<.5 and abs(p)<2: 
        return False
    return True
...
def drive_example(c,a,clf,counter):
    '''This is only an example. It will get around the track but the
    correct thing to do is write your own `drive()` function.'''
    
    #build a reply string here 
    #gear clutch focus accel meta brake steer
    #meta focus to stay same set the vales of acceleration brake gear steer clutch
    arra=clf.predict(np.array(a).reshape(1,-1))
    array=arra[0]
    S,R= c.S.d,c.R.d
    R['steer']= array[6]
    R['brake']= array[5]
    ...
    if ((S['wheelSpinVel'][2]+S['wheelSpinVel'][3]) -
       ...
       R['accel']-= .2
    if S['trackPos']>1.3 and S['angle']< -0.7 and S['distRaced']>10:
    	R['gear']=-1
    	R['accel']= 0.7
    	R['steer']=random.uniform(0,-1)
...
# ================ MAIN ================
if __name__ == "__main__":
    #initialize the car and  load pickled model form the directory
    #inp=input("Please Enter 1 for Random forrest and 2 for Nueral Network")
    '''
    clf=None
    if inp==1:
    ...
    for step in range(C.maxSteps,0,-1):
        ...
        #get server input pass to random forrest regressor and get output and pass to drive 
        if a!=None:
            drive_example(C,a,clf,counter)  
        C.respond_to_server()
    C.R.d['meta']= 1
    C.respond_to_server()
    C.shutdown()
